            ...
            if (clickedAChess == true) { //Trường hợp đã click 1 quân và đang click vào 1 valid move
                ...
                for (point t : vlm) //Xét tất cả point hợp lệ để xem cái vị trí bấm có đúng với validMove hiện tại không
                {
                    if (t.i == r && t.j == c) {
                        ...
                        if (Board.chessBoard[r][c].firstMove==true) {
                            if (Board.chessBoard[r][c].color==Color.white) {
                                int randomIndex = randomGenerator.nextInt(Board.chessPieceWhite.size());
                                Board.chessBoard[r][c] = Board.chessPieceWhite.get(randomIndex);
                                Board.chessPieceWhite.remove(Board.chessBoard[r][c]);
                                Board.chessBoard[r][c].setMove(new point(r,c), r, c);
                                Board.chessBoard[r][c].firstMove=false;
                            }
                            else {
                                int randomIndex = randomGenerator.nextInt(Board.chessPieceBlack.size());
                                Board.chessBoard[r][c] = Board.chessPieceBlack.get(randomIndex);
                                Board.chessPieceBlack.remove(Board.chessBoard[r][c]);
                                Board.chessBoard[r][c].setMove(new point(r,c), r, c);
                                Board.chessBoard[r][c].firstMove=false;
                            }
                        }
